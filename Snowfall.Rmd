...
> - Getting data from an API (`httr`): <br> Webinar:  https://www.rstudio.com/resources/webinars/extracting-data-from-the-web-part-1/ <br> Slides: https://github.com/rstudio/webinars/blob/master/31-Web-APIs/01-APIs.pdf

> - Read files directly from web site

> - Download files from web site (either manually or with a script)

> - Scrape web pages (`rvest`): <br> Webinar: https://www.rstudio.com/resources/webinars/extracting-data-from-the-web-part-2/ <br> Slides: https://github.com/rstudio/webinars/blob/master/32-Web-Scraping/02-Web-Scraping.pdf
...
snow2 <- snow %>% select(`Feb 1`:`Feb 28`) %>% 
  mutate_all(funs(ifelse(. == "T", .005, .) %>% as.numeric()))
```
